...
export default function App() {
  ...
  // ── ui state ───────────────────────────────────────────────
  const [modal, setModal] = useState(null)   // 'search' | 'profile' | 'settings' | null
  ...
  const exportCSV = () => {
    const rows = [
      ['ID', 'Name', 'Department', 'Semester', 'GPA', 'Attendance', 'Grade', 'Status'],
      ...filtered.map(s => [s.id, s.name, s.dept, s.sem, s.gpa, s.att, s.grade, s.status]),
    ]
  ...
  const resetFilters = () => {
    setDeptF('All'); setSemF('All'); setSearch('')
  ...
  const handleSbSelect = (index, label) => {
    setActiveSb(index)
    if (label === 'Settings') { setModal('settings'); return }
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', height: '100vh', overflow: 'hidden', position: 'relative' }}>
        ...
        {/* ── CONTENT ── */}
        <main style={{ flex: 1, padding: '22px 24px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 16, position: 'relative' }}>
          ...
          <div style={{
            ...
            <span style={{ fontSize: 11.5, color: 'var(--muted)', fontWeight: 500, marginLeft: 8 }}>Sem</span>
            ...
            <div style={{ marginLeft: 'auto', position: 'relative' }}>
              <svg style={{ position: 'absolute', left: 9, top: '50%', transform: 'translateY(-50%)', width: 13, height: 13, color: 'var(--muted2)', pointerEvents: 'none' }}
              ...
              <input
                ...
                style={{
                  padding: '5px 12px 5px 28px', borderRadius: 20,
      ...
      {modal === 'profile' && (
        <Modal onClose={() => setModal(null)}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 18 }}>
            <div style={{
              width: 48, height: 48, borderRadius: '50%', background: '#d4c9b8',
      ...
      {modal === 'settings' && (
        <Modal onClose={() => setModal(null)}>
          ...
          {[['Academic Year', '2024–25'], ['Default View', 'All Departments'], ['Records per page', '8'], ['Theme', 'Light']].map(([k, v]) => (
            <div key={k} className="csv-row">
              <span style={{ color: 'var(--muted)' }}>{k}</span>
